...
#basic script ot check the aws ec2 instance running with nginx and docker installed
#public DNS names should be enabled in the VPC with the ec2 instance
#run with aws-profile name
...
if [ -z "$instance_address" ]; then
    echo "Cannot find instance address for given instance ID: $instance_id"
    exit 1
fi
